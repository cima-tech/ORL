...
function getNavGroupHTML() {
    ...
    // Extracción de datos del JSON complejo
    const profile = STATE.currentUser.profile;
    const fullName = `${profile.title} ${profile.firstname} ${profile.lastname}`;
    const roleDisplay = profile.title_line_1 || profile.role;
    
    // Iniciales
    const initials = profile.firstname ? profile.firstname[0] + (profile.lastname ? profile.lastname[0] : '') : "DR";
...
function getPreviewGroupHTML() {
    ...
    return `<div class="v-divider"></div><div class="toolbar-group animate-fade" style="background:rgba(16, 185, 129, 0.1); border-radius:12px; padding:0 10px;"><div class="icon-row"><div style="display:flex; flex-direction:column; gap:2px; align-items:center;"><input type="range" id="zoomRange" min="40" max="130" step="5" value="70" style="width:60px; height:4px;"><span id="zoomVal" style="font-size:0.6rem; opacity:0.8;">70%</span></div><button id="btnToggleSign" class="icon-btn" title="Firmar" style="color:${signColor}"><i class="bi ${signIcon}"></i></button><div style="width:1px; height:20px; background:rgba(255,255,255,0.2);"></div><button id="btnRefresh" class="icon-btn" title="Actualizar"><i class="bi bi-arrow-clockwise"></i></button><button id="btnOpenExport" class="icon-btn" title="Exportar" style="background:var(--success); border-color:var(--success);"><i class="bi bi-share-fill"></i></button><button id="btnExitPreview" class="icon-btn" title="Cerrar Preview" style="margin-left:5px; background:rgba(255,255,255,0.1);"><i class="bi bi-x"></i></button></div><span class="group-label" style="color:var(--success);">Vista Previa</span></div>`;
}
