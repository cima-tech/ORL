...
// ==========================================
// A. LOGICA DE UI (DROPDOWNS EN CONSULTA)
// ==========================================
...
// ==========================================
// B. GENERADORES DE HTML (DOCUMENTOS)
// ==========================================
...
export function buildReportHTML(card) {
    ...
    if (Object.values(examMap).some(v => v && v.trim())) {
        ...
        for (const [k, v] of Object.entries(examMap)) {
            if (v && v.trim()) examHTML += `&bull; <strong>${k}:</strong> ${v}<br>`;
        }
        examHTML += '</div>';
    }
    ...
    CIMA_DATA.ADDITIONAL_STUDIES.forEach(name => {
        const inputClass = `.txt-study-${name.toLowerCase().replace(/ /g, '-')}`;
        const res = card.querySelector(inputClass)?.value || '';
        if (res) studiesHTML += `<div><strong>${name}:</strong> ${res}</div>`;
    });
    ...
    <div class="doc-page doc-letter">
        ...
        <div class="doc-wrap">
            <div style="text-align: center; margin-bottom: 30px;">
                <h2 style="color: #000; margin: 0; font-size: 1.2rem; text-transform: uppercase;">Informe Médico</h2>
            ...
            <div style="font-weight: 600; margin-bottom: 10px; border-bottom: 2px solid #000; padding-bottom: 5px;">
                PACIENTE: ${d.pNombre || '—'} <br>
                <span style="font-weight: 400; font-size: 0.9em;">ID: ${d.doc}</span>
            </div>
            
            <div style="margin-bottom: 20px; text-align: right; font-size: 0.9em; color: #000;">
                <strong>Caracas, ${fmtDateTime(d.dateISO)}</strong>
            </div>
            ...
            <div style="margin-top: auto; display:flex; justify-content:center; gap:20px; align-items:flex-end; min-height: 100px;">
                <div>${d.signImg}</div>
                <div>${d.stampImg}</div>
            </div>
    ...
    const firmaBlock = `
        <div style="height:120px; position:relative; display:flex; flex-direction:column; justify-content:flex-end; align-items:center; margin-top:auto;">
            ...
            <div style="text-align:center; font-size:0.75rem; color:#000; line-height:1.2;">
                <div style="font-weight:bold; font-size:0.9rem; border-top:1px solid #000; padding-top:4px; width:220px; margin:0 auto 2px auto;">${d.drName}</div>
                ${d.drSpec}<br><span style="font-size:0.7rem;">${d.drCode}</span>
            </div>
        </div>`;
    ...
    <div class="doc-page doc-letter land" style="padding:30px 40px; display:grid; grid-template-columns: 1fr 1fr; gap:50px;">
        <div style="display:flex; flex-direction:column; height:100%; border-right:1px dashed #cbd5e1; padding-right:25px;">
            ...
            <div style="display:flex; justify-content:space-between; align-items:flex-end; border-bottom:2px solid #333; margin-bottom:15px; padding-bottom:5px;">
                ...
                <div style="font-size:0.85rem;">${fmtDate(d.dateISO)}</div>
            </div>
